...
## 2026-06-13 - [CSP Unification Across Secondary Pages and .htaccess]
**Vulnerability:** Secondary HTML files (like 404.html, projects.html, etc.) and the global .htaccess file had overly permissive CSP directives, specifically allowing 'unsafe-inline' in script-src which enables XSS attacks.
**Learning:** When unifying CSP across an application, it is critical to evaluate the specific needs of all pages. Applying exact sha256 hashes derived strictly from one page (e.g., index.html) to the global .htaccess policy blocks unique inline scripts on other pages, breaking functionality. The .htaccess CSP must be a superset of all page-specific requirements.
**Prevention:** Before globally enforcing a stricter CSP, always audit inline scripts across all entry points. Build a global policy that safely accommodates all required external domains and legitimate inline script hashes, or externalize all inline scripts to avoid using hashes altogether.
