...
## 2025-02-25 - [CSP Hardening and Inline Script Consolidation for Modern Pages]
**Vulnerability:** The website's modern pages (`modern.html`, `projects.html`, `publications.html`, `service.html`, `experience.html`, `404.html`) had a Content Security Policy (CSP) that allowed `'unsafe-inline'` in `script-src`, leaving it vulnerable to Cross-Site Scripting (XSS).
**Learning:** Moving non-critical inline scripts to a single external file (`assets/js/home.js`) enables a stricter CSP. Inline event handlers must be refactored into external scripts.
**Prevention:** Design new pages with strict CSP in mind. Avoid inline scripts and event handlers.
