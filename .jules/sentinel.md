...
## 2026-06-02 - [Securing Dynamically Injected Iframes]
**Vulnerability:** The application was dynamically creating and injecting a third-party iframe (YouTube embed) via JavaScript without explicitly setting the `sandbox` attribute. This potentially allows the third-party content unrestricted privileges within the iframe context, increasing the risk of malicious behavior if the third-party is compromised.
**Learning:** Even well-known third-party embeds should follow the principle of least privilege. Dynamically created iframes require the same security considerations (like `sandbox` restrictions) as statically defined iframes in HTML. When sandboxing a YouTube embed, specific permissions (`allow-scripts allow-popups allow-presentation allow-same-origin`) are required for the player to function correctly without granting full unrestricted access.
**Prevention:** Always apply the `sandbox` attribute to `<iframe>` elements, whether they are static HTML or dynamically created via `document.createElement('iframe')`. Start with the most restrictive policy and add only the specific `allow-*` flags necessary for the third-party content to function.
