...
## 2026-06-11 - [Restricting Dynamic Third-Party Iframes]
**Vulnerability:** The website dynamically creates an `iframe` in `assets/js/main.js` to embed a YouTube video without any `sandbox` attribute. This allowed the embedded third-party content unrestricted privileges within its frame, potentially allowing it to execute malicious scripts or navigate the top-level browsing context (frame busting).
**Learning:** Even trusted third-party embeds (like YouTube) should be sandboxed to follow the principle of least privilege. When sandboxing YouTube, `allow-same-origin` is required for the player to function properly alongside `allow-scripts allow-popups allow-presentation`. Dynamically created iframes require the same security considerations as static HTML iframes.
**Prevention:** Always apply the `sandbox` attribute to `iframe` elements, regardless of whether they are statically written in HTML or dynamically created via JavaScript. Start with an empty sandbox and only whitelist necessary permissions.
