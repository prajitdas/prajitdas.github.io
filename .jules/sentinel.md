...
## 2026-07-07 - [Strict Sandbox for Dynamic Iframes]
**Vulnerability:** Dynamically generated third-party iframes (e.g., YouTube video embeds in `main.js`) lacked strict sandboxing, potentially exposing the application to cross-origin privilege escalation or window hijacking if the third-party content is compromised.
**Learning:** Third-party embeds dynamically injected via JavaScript bypass static HTML security scanning and require explicit `sandbox` attributes configured directly on the `HTMLIFrameElement` before injection.
**Prevention:** Always apply the principle of least privilege using the `sandbox` property (e.g., `allow-scripts allow-popups allow-presentation allow-same-origin`) when dynamically creating iframes, explicitly avoiding `allow-top-navigation` unless strictly required.
