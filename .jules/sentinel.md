...
## 2026-06-25 - [Sandboxing Third-Party Iframes]
**Vulnerability:** Dynamically created YouTube iframes lacked the `sandbox` attribute. While they were embedded securely without `allow-same-origin` initially, the lack of explicit sandboxing meant they theoretically possessed full iframe capabilities, which could be exploited if a malicious actor found a way to inject or redirect the iframe source.
**Learning:** Defense in depth dictates that third-party content (even trusted sources like YouTube) embedded via iframes should be explicitly restricted. Using the `sandbox` attribute minimizes the attack surface by defaulting to a highly restricted state and only whitelisting necessary privileges. For YouTube embeds, `allow-scripts`, `allow-popups`, `allow-presentation`, and crucially `allow-same-origin` are needed to allow the player to function properly.
**Prevention:** Always apply the `sandbox` attribute to `<iframe>` elements (both statically and dynamically created) when embedding external content, explicitly allowing only the minimum required privileges for functionality.
