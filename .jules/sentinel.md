...
## 2025-02-24 - [CSP Hardening: Removing unsafe-inline from scripts]
**Vulnerability:** The website's Content Security Policy (CSP) allowed `'unsafe-inline'` in `script-src` and `default-src` across all HTML files and the `.htaccess` file, leaving the application vulnerable to Cross-Site Scripting (XSS).
**Learning:** Extracting inline scripts (such as the "Read More" and "Email Obfuscation" logic in `index.html`) into external files (like `assets/js/home.js`) enables the removal of `'unsafe-inline'` from the `script-src` CSP directive. However, care must be taken with `default-src` in `.htaccess`; if it is tightened, an explicit `style-src` directive allowing `'unsafe-inline'` must be added if inline styles are still used, to avoid visual regressions.
**Prevention:** Avoid inline scripts entirely. Externalize all JavaScript logic. Ensure that CSP tightening in headers matches the capabilities defined in HTML meta tags, explicitly defining fallback directives like `style-src` when `default-src` is modified.
