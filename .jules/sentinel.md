...
## 2026-05-28 - [Secure Dynamic Iframe Privileges]
**Vulnerability:** The YouTube video embed generated dynamically in `assets/js/main.js` lacked a `sandbox` attribute. This allowed the embedded third-party content unrestricted privileges within the iframe, increasing the risk if the embed provider was compromised or served malicious content.
**Learning:** Defense-in-depth requires restricting privileges wherever possible, including dynamically created elements. Using the `sandbox` attribute on iframes is a critical security practice, especially for third-party embeds. For YouTube embeds, `allow-scripts allow-popups allow-presentation allow-same-origin` provides the necessary functionality while preventing more dangerous actions like top-level navigation.
**Prevention:** Always include a `sandbox` attribute when creating `<iframe>` elements dynamically via JavaScript, granting only the explicitly required permissions.
