...
## 2026-06-12 - [Synchronizing Strict CSP Across HTML and HTTP Headers]
**Vulnerability:** The `.htaccess` file contained a weaker CSP allowing `'unsafe-inline'` in `default-src`. Furthermore, secondary HTML files (`projects.html`, `modern.html`, etc.) had mismatched `<meta>` CSP tags that allowed `'unsafe-inline'` in `script-src`, creating multiple XSS vectors across the site.
**Learning:** CSP rules defined in HTTP headers and HTML `<meta>` tags operate cumulatively, but inconsistent application across files creates security loopholes. Whitelisting required domains and specific script hashes across *all* CSP definitions is necessary to maintain both security and functionality.
**Prevention:** Maintain a single source of truth for CSP strings. When updating CSP (e.g., adding a new script hash or domain), ensure both the `.htaccess` and all HTML `<meta>` tags are perfectly synchronized and strictly avoid `'unsafe-inline'` in `script-src` and `default-src`.
