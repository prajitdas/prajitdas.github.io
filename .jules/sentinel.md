...
## 2025-04-07 - [CSP Header and Inline Styles in Apache .htaccess]
**Vulnerability:** The `.htaccess` file explicitly defines a strict Content Security Policy but retains `'unsafe-inline'` specifically for the `style-src` directive to prevent breaking existing CSS functionality while tightening `default-src`. If `style-src` is omitted when redefining CSP via Apache headers, `default-src` handles style parsing, which can break site UI and scripts (e.g. jQuery `.show()` / `.hide()`) globally across the application if `'unsafe-inline'` is absent.
**Learning:** Because browsers enforce the strictest intersection of all active CSPs, the `.htaccess` header will override HTML meta tags. When removing `'unsafe-inline'` from `default-src` in `.htaccess`, `style-src` MUST be explicitly declared allowing `'unsafe-inline'` to maintain frontend styles.
**Prevention:** When adjusting CSP rules in `.htaccess`, test comprehensively across pages with inline styles. explicitly declare each directive to avoid unintentionally tightening policies via `default-src`.
