...
## 2026-06-27 - [Hardened Content Security Policy]
**Vulnerability:** The default Content-Security-Policy globally allowed `'unsafe-inline'` across scripts inside multiple pages including `.htaccess`'s `default-src` directive which makes the application susceptible to Cross-Site Scripting (XSS).
**Learning:** Removing `'unsafe-inline'` from `default-src` without defining `style-src` implicitly forces `style-src` to inherit `default-src`'s restrictions, breaking site styles. `script-src` and `style-src` must be handled explicitly.
**Prevention:** The `.htaccess` CSP explicitly separates `script-src` (omitting `'unsafe-inline'`) and `style-src` (allowing `'unsafe-inline'`) to globally prevent inline scripts without breaking required inline styles, effectively overriding the `default-src` fallback behavior.
