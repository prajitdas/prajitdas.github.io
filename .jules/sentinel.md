...
## 2026-04-16 - [YouTube Iframe Sandbox Hardening]
**Vulnerability:** Dynamically generated YouTube iframes were missing the `sandbox` attribute, potentially allowing malicious scripts within the iframe to execute with excessive privileges in the parent page's context.
**Learning:** Third-party embedded content, especially dynamically created iframes, should always be restricted using the `sandbox` attribute. For YouTube embeds specifically, `allow-same-origin` is required for the player to function properly, along with `allow-scripts`, `allow-popups`, and `allow-presentation`.
**Prevention:** When dynamically creating iframes or embedding third-party content, consistently apply the `sandbox` attribute with the principle of least privilege, only granting necessary permissions for the embed to function.
