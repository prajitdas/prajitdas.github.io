...
## 2025-05-16 - [CSP Hardening on modern.html and secondary pages]
**Vulnerability:** Secondary HTML pages (like `modern.html`, `projects.html`, `404.html`, etc.) and the global `.htaccess` file had a Content Security Policy (CSP) that allowed `'unsafe-inline'` for `script-src`. `modern.html` specifically had an inline script for UI toggling and email obfuscation, preventing the enforcement of a strict CSP.
**Learning:** To eliminate `'unsafe-inline'`, all scripts must be either externalized or their exact SHA-256 hashes must be allowed in the CSP. Externalizing scripts (like creating `assets/js/modern.js`) is the preferred and scalable approach.
**Prevention:** Always place JavaScript logic in external `.js` files and load them using `<script src="..." defer></script>`. Ensure the global `.htaccess` CSP restricts `script-src` to `'self'` (and known hashes for critical inline scripts) while completely avoiding `'unsafe-inline'` for scripts.
