...
## 2026-05-01 - [Third-Party Iframe Privilege Restriction]
**Vulnerability:** Dynamically generated third-party iframes (e.g., YouTube embeds) in `assets/js/main.js` lacked the `sandbox` attribute, potentially allowing external content broader execution privileges than necessary.
**Learning:** Even well-known third-party embeds should be sandboxed to restrict their capabilities. However, when sandboxing YouTube iframes, `allow-same-origin` is required alongside `allow-scripts`, `allow-popups`, and `allow-presentation` for the video player to function correctly.
**Prevention:** Always include the `sandbox` attribute when dynamically creating iframes or embedding external content, explicitly defining the minimum required permissions for the content to function safely.
