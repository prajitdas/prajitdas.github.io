...
## 2026-07-06 - [Defense in Depth Security Enhancements]
**Vulnerability:** The application had several potential attack vectors: missing strict sandbox parameters on dynamically created YouTube iframes, an active (though deprecated) XSS-Auditor header that can be abused, and a permissive 'unsafe-inline' CSP script-src directive in secondary HTML files and global .htaccess fallback.
**Learning:** Implementing security requires a defense-in-depth approach spanning multiple configurations. First, dynamically created iframes must explicitly define sandbox attributes (like 'allow-scripts allow-popups allow-presentation allow-same-origin' for YouTube). Second, disabling X-XSS-Protection mitigates risks of browser auditor bypass/exploitation. Third, keeping CSP strictly enforced across all pages—not just index.html—prevents XSS execution in less monitored areas of the site.
**Prevention:** When dynamically injecting DOM elements, especially third-party iframes, enforce strict sandbox rules. Avoid outdated headers like X-XSS-Protection. Ensure strict CSP headers, omitting 'unsafe-inline' for scripts, are applied consistently globally (.htaccess) and locally (meta tags).
