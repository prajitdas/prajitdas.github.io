...
## 2026-05-29 - [Secure Third-Party Iframes]
**Vulnerability:** Dynamically created third-party iframes (e.g., YouTube embeds in `assets/js/main.js`) were inserted without restricting their privileges using the `sandbox` attribute. This is a potential risk as it gives the third-party content full access to its origin and potentially other capabilities.
**Learning:** Always implement defense in depth for third-party embeds by using the `sandbox` attribute to explicitly allow only the necessary privileges. For YouTube, `allow-scripts allow-popups allow-presentation allow-same-origin` is the required minimal set of permissions.
**Prevention:** Include the `sandbox` attribute when creating iframes, whether statically in HTML or dynamically in JavaScript, applying the principle of least privilege.
