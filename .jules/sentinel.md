...
## 2026-06-01 - [Synchronizing CSP Headers and Meta Tags]
**Vulnerability:** A vulnerability assessment revealed a medium-risk issue related to inline JavaScript and the Content Security Policy (CSP). While the CSP in `.htaccess` was tightened to allow specific inline scripts via `sha256` hashes, the corresponding `<meta>` tags in HTML files (`index.html`, `404.html`, etc.) were not updated to match.
**Learning:** CSP rules are cumulative. If multiple policies are delivered (e.g., via HTTP headers and `<meta>` tags), a resource must pass *all* of them to load. When updating CSP to allow specific inline scripts using hashes, those hashes must be added to all CSP definitions (headers and meta tags) to prevent legitimate scripts from being blocked, which causes functional regressions.
**Prevention:** Always synchronize CSP definitions across all delivery mechanisms (`.htaccess`, HTTP headers, and HTML `<meta>` tags). When generating a hash to permit an inline script, ensure it is added everywhere the CSP is defined.
