...
## 2026-06-20 - [Strict Sandbox Policy for Third-Party Iframes]
**Vulnerability:** Dynamically created third-party `iframe` elements (e.g., for YouTube video embeds) lacked the `sandbox` attribute, allowing them full privileges over the page. This increases the risk of Cross-Site Scripting (XSS) and other malicious behaviors if the third-party content is compromised or acts unexpectedly.
**Learning:** All third-party `iframe` elements must implement strict `sandbox` policies. The principle of least privilege should be applied, granting only the specific permissions needed. For example, a YouTube embed requires `allow-scripts`, `allow-popups`, `allow-presentation`, and `allow-same-origin` to function correctly, but other privileges should remain restricted.
**Prevention:** Always add the `sandbox` attribute to `iframe` elements, especially those loading external content dynamically. Regularly review iframe permissions and ensure they are as restrictive as possible.
