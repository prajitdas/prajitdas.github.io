...
## 2026-04-03 - Strict Iframe Sandboxing
**Vulnerability:** Dynamically injected iframes (e.g., YouTube embeds) lacked a `sandbox` attribute, potentially exposing the parent context to malicious scripts or unwanted navigation if the third-party content is compromised.
**Learning:** Using `sandbox="allow-scripts allow-same-origin ..."` negates the security benefits of sandboxing because the iframe can use scripts to access the DOM of the same origin and simply remove the sandbox attribute itself. Thus, it's a critical security practice to omit `allow-same-origin` unless absolutely necessary when `allow-scripts` is present.
**Prevention:** Always add a strict `sandbox` attribute to dynamically created `iframe` elements (e.g., `allow-scripts allow-popups allow-presentation`) and explicitly exclude `allow-same-origin` when scripts are allowed to ensure robust isolation of third-party content.
