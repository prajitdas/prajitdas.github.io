...
## 2026-06-30 - Fix ACME Challenge Blocking in .htaccess
**Vulnerability:** The hidden file blocking rule `RewriteRule (^|/)\. - [F,L]` in `.htaccess` blocks all hidden directories indiscriminately, including the `.well-known/` directory required for Let's Encrypt SSL certificate renewals (ACME challenges). This could lead to expired SSL certificates, presenting a security risk to users.
**Learning:** Security rules intended to block access to hidden files must be carefully scoped so they do not inadvertently break critical infrastructure mechanisms like automated SSL renewals.
**Prevention:** Use a negative lookahead regex such as `RewriteRule (^|/)\.(?!well-known/) - [F,L]` to explicitly exempt directories that require public access for operational security reasons.
