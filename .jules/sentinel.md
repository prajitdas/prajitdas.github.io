...
## 2025-05-24 - [Strict CSP Enforcement]
**Vulnerability:** The application used `script-src 'self' 'unsafe-inline'` in its Content Security Policy (CSP) across multiple HTML files and `.htaccess`, neutralizing XSS protections and allowing arbitrary inline scripts to execute.
**Learning:** Legacy inline event handlers and inline `<script>` tags for UI interactions (like "Read More" toggles or email obfuscation) prevented the application from enforcing a strict CSP. Extracting these into external files (`assets/js/index-interactions.js`) is required to safely remove `'unsafe-inline'`.
**Prevention:** Avoid writing inline `<script>` tags or inline event handlers (`onclick`, etc.). Always place JavaScript in external `.js` files and use event listeners attached via DOM queries to ensure compatibility with a strict `script-src 'self'` CSP.
