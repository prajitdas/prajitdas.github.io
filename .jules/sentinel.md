...
## 2026-07-04 - [Enforcing Least Privilege on Dynamically Created Iframes]
**Vulnerability:** The JavaScript codebase dynamically created iframes to embed third-party content (e.g., YouTube videos) without explicitly restricting the privileges of those iframes using the `sandbox` attribute. This could allow malicious third-party content to execute scripts, create popups, or navigate the top-level window, posing a security risk.
**Learning:** Even when embedding trusted third-party content like YouTube videos, applying the principle of least privilege is crucial. The `sandbox` attribute must be explicitly defined with the minimum required permissions (`allow-scripts allow-popups allow-presentation allow-same-origin`) to mitigate potential risks if the third-party source is compromised or serves unexpected content.
**Prevention:** Always include the `sandbox` attribute with explicitly defined permissions when dynamically creating or statically defining iframes for third-party embeds.
