...
## 2026-06-09 - [Synchronize Content Security Policy]
**Vulnerability:** The Content Security Policy (CSP) set in `.htaccess` and secondary HTML pages (e.g., `404.html`, `projects.html`) included `'unsafe-inline'` in `default-src` and `script-src` directives. This exposed the site to Cross-Site Scripting (XSS) attacks by allowing arbitrary inline scripts, despite `index.html` having a strict policy using explicit `sha256` hashes.
**Learning:** CSP rules are cumulative. When policies are defined in both HTTP headers (`.htaccess`) and HTML `<meta>` tags, all resources must pass all policies. An overly permissive policy in `.htaccess` or secondary pages undermines the strict policy in the main entry point, creating significant security gaps across the site.
**Prevention:** Maintain a single, strictly defined CSP and ensure it is uniformly applied across all configuration files (like `.htaccess`) and `<meta>` tags in all HTML files. Always avoid `'unsafe-inline'` for scripts and use cryptographic hashes or nonces instead.
