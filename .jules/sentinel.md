...
## 2026-07-07 - [Sandboxing Third-Party Iframes]
**Vulnerability:** The application was dynamically creating and embedding YouTube iframes without the `sandbox` attribute. This allowed the embedded content full privileges (e.g., top-level navigation, executing arbitrary scripts if the source was compromised), which poses a security risk.
**Learning:** Third-party embedded content, especially dynamically created iframes, should always operate under the principle of least privilege. The `sandbox` attribute is crucial for restricting what the embedded content can do. However, care must be taken to add the specific `allow-*` flags required for the legitimate functionality of the iframe (like `allow-scripts`, `allow-same-origin`, etc. for YouTube).
**Prevention:** Whenever embedding third-party content via `<iframe>`, whether statically in HTML or dynamically via JavaScript, always include a strict `sandbox` attribute and only grant the minimum necessary permissions required for the embed to function.
